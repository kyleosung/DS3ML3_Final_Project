...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(64 + 1 + 1, 1024)  # 64 input nodes + 1 input node for turn + 1 node for check
...
def custom_loss(dataset, output, target, fen, illegal_move_penalty=5000.0):
    ...
    move = dataset.output_to_move(output)
    ...
    for epoch in range(num_epochs):
        ...
        try:
            for i, data in enumerate(data_loader):
                ...
                # Forward pass and backward pass for each observation
                for j in range(inputs.shape[0]):
                    input = inputs[j]
                    label = labels[j].unsqueeze(0)

                    input_with_turn = torch.cat((input, turns[j].unsqueeze(0), checks[j].unsqueeze(0)), dim=0).to(device)

                    # Forward pass
                    output = model(input_with_turn)
                    loss = criterion(dataset, output, label, fen[j])
...
def predict(model, fen, white_to_move):
    ...
    fen_tensor = ChessIterableDataset(None, None).fen_to_tensor(fen)

    move_tensor = torch.tensor(white_to_move).unsqueeze(0)
    check_tensor = torch.tensor(is_check(fen)).unsqueeze(0)
